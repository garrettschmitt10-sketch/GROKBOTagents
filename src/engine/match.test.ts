...
import { deployAllowed } from "./pathing";
import { ARENA_H, HAND_SIZE, MAX_ELIXIR } from "./types";

describe("Aetherlane match engine", () => {
  ...
  it("refuses plays that cost more aether than available", () => {
  ...
  it("keeps ground troops off the river except on bridges", () => {
    expect(deployAllowed(0, "ashblade", 9, 16, { left: false, right: false })).toBe(false);
    expect(deployAllowed(0, "ashblade", 3.8, 20, { left: false, right: false })).toBe(true);
    expect(deployAllowed(0, "riftburst", 9, 8, { left: false, right: false })).toBe(true);
    expect(deployAllowed(0, "ashblade", 4, 8, { left: false, right: false })).toBe(false);
    expect(deployAllowed(0, "ashblade", 4, 8, { left: true, right: false })).toBe(true);
  });

  it("lets the bot spend aether under the same rules", () => {
  ...
  it("simulates a full match without NaNs or elixir overflow", () => {
  ...
  it("has the bot spend aether and field units", () => {
